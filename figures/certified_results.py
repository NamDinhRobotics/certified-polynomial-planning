...
def artifact():
    return json.loads((ROOT/'artifacts/review_numpy_solver.json').read_text())
...
def compute(x):
    ...
    s['paired_median_ratio']=float(np.median([r['reference']['total_ms']/r['factor']['total_ms'] for r in rows]))
...
\begin{table}[!t]
\caption{Explicit NumPy backend, same F2 SDP: 3,000 calls per arm, including cold calls and fallback.
