...
from exact_sdp_bounds import ExactSDPBounds,fraction_record,rational_array
from constructive_recovery import coefficients,encode
...
def service(f,exact,green,obs,budget=12,warm=True):
 ...
 a=dict(ok=False,status='NO_NUMERICAL_RESULT');bound=None;upper_point=None
 if numerical['ok']:
  if numerical['source']=='factor':Y,v,*_=f.unpack(numerical['z']);point=None
  else:
   Y,V=conic_point(numerical['conic']);point=(Y,V);v=None
   upper_point=[encode(rational_array(x)) for x in point]
 ...
 payload=json.dumps(clean(dict(physical=a,numerical=numerical,exact=bound,upper_point=upper_point)),separators=(',',':'),allow_nan=False)
 ...
 return dict(numerical=numerical,upper_point=upper_point,arms={'green':a},exact=bound,physical_ok=a['ok'],objective_ok=objective_ok,
